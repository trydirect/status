...
1. **Dashboard enqueues** → POST to `/api/v1/commands/enqueue`
2. **Agent polling** → GET to `/api/v1/commands/wait/{hash}` (waiting)
...
6. **Agent reports** → POST to `/api/v1/commands/report` with result
...
- [API_SPEC.md](API_SPEC.md) - Full `/api/v1/commands/*` endpoint specification
