...
pub fn verify_signature(headers: &HeaderMap, body: &[u8], key: &str, max_skew_secs: i64) -> Result<()> {
    ...
    if skew > max_skew_secs { return Err(anyhow!("stale request (timestamp skew)")); }
    ...
    // Compute expected
    let mut mac = HmacSha256::new_from_slice(key.as_bytes()).map_err(|_| anyhow!("invalid hmac key"))?;
    mac.update(body);
    let expected = mac.finalize().into_bytes();

    verify_slices_are_equal(&provided, expected.as_slice()).map_err(|_| anyhow!("signature mismatch"))?;
    Ok(())
}
